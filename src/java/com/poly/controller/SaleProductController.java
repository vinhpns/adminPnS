...
public class SaleProductController {
    ...
    public String initiate(ModelMap model, HttpSession session) {
        if (Objects.equals(pSER.checkLogin(session), Boolean.FALSE)) {
        ...
        model.addAttribute(ProductConstant.SALE_PRODUCTS_KEYS, pSER.getListProduct(ProductConstant.SALE_PRODUCT, session));
        return ProductConstant.RETURN_SALE_PRODUCT_PAGE;
    }

    @RequestMapping(params = "approved", method = RequestMethod.GET)
    public String approveByRole(ModelMap model, HttpSession session, @RequestParam("id") int id) {
        if (Objects.equals(pSER.checkLogin(session), Boolean.FALSE)) {
            model.addAttribute(ConstantManager.ERROR_POPUP, ConstantManager.NO_ACCEPTED_LOGIN);
            return accController.initiate(model, session);
        }
        Boolean checkStatus = pSER.updatApprovedStatus(session, id);
    ...
    @RequestMapping(params = "returnStatus", method = RequestMethod.POST)
    ...
    public String insertAuctionProduct(ModelMap model, HttpSession session,
        ...
        if (Objects.equals(pSER.insertEcomerceProduct(product, session), Boolean.FALSE)) {
        ...
        if (Objects.equals(pSER.checkProductFullApproved(productGetId.getId()), Boolean.TRUE)) {
            insertBid = pSER.insertSelling(productGetId.getId());
        }
        if (Objects.equals(insertBid, Boolean.TRUE)) {
            pSER.updateActive(productGetId.getId());
        }
        return initiate(model, session);
    }

    @RequestMapping(params = "updateProduct", method = RequestMethod.POST)
    public String updateAuctionProduct(ModelMap model, HttpSession session,
            @ModelAttribute(ProductConstant.KEY_SALE_PRODUCT) Product product,
            @RequestParam(ProductConstant.URL_PARAM) String url,
            @RequestParam(ProductConstant.IMG_1_PARAM) MultipartFile img1,
            @RequestParam(ProductConstant.IMG_2_PARAM) MultipartFile img2,
            @RequestParam(ProductConstant.IMG_3_PARAM) MultipartFile img3,
            @RequestParam(ProductConstant.IMG_4_PARAM) MultipartFile img4,
            @RequestParam(ProductConstant.IMG_5_PARAM) MultipartFile img5) {
        pSER.updateProduct(product);
        Utils uploadImg = new Utils();
        List<String> listNames = new ArrayList<>();
        List<MultipartFile> listFiles = new ArrayList<>();
        ...
        if (img1 != null) {
            String img1Name = uploadImg.randomCodeImg() + img1.getOriginalFilename();
            pImg = pSER.getIdFromLink(getImg.get(0).getLink());
            if (pImg == null) {
                pSER.insertProductImg(ProductConstant.URL_SALE_SERVER + img1Name, pImg.get(0).getProductId());
            }
            pSER.updateAuctionProductImg(ProductConstant.URL_SALE_SERVER + img1Name, pImg.get(0).getId(), pImg.get(0).getProductId());
            listNames.add(img1Name);
            listFiles.add(img1);
        }
        if (img2 != null) {
            String img2Name = uploadImg.randomCodeImg() + img2.getOriginalFilename();
            pImg = pSER.getIdFromLink(getImg.get(0).getLink());
            if (pImg == null) {
                pSER.insertProductImg(ProductConstant.URL_SALE_SERVER + img2Name, pImg.get(0).getProductId());
            }
            pSER.updateAuctionProductImg(ProductConstant.URL_SALE_SERVER + img2Name, pImg.get(0).getId(), pImg.get(0).getProductId());
            listNames.add(img2Name);
            listFiles.add(img2);
        }
        if (img3 != null) {
            String img3Name = uploadImg.randomCodeImg() + img3.getOriginalFilename();
            pImg = pSER.getIdFromLink(getImg.get(0).getLink());
            if (pImg == null) {
                pSER.insertProductImg(ProductConstant.URL_SALE_SERVER + img3Name, pImg.get(0).getProductId());
            }
            pSER.updateAuctionProductImg(ProductConstant.URL_SALE_SERVER + img3Name, pImg.get(0).getId(), pImg.get(0).getProductId());
            listNames.add(img3Name);
            listFiles.add(img3);
        }
        if (img4 != null) {
            String img4Name = uploadImg.randomCodeImg() + img4.getOriginalFilename();
            pImg = pSER.getIdFromLink(getImg.get(0).getLink());
            if (pImg == null) {
                pSER.insertProductImg(ProductConstant.URL_SALE_SERVER + img4Name, pImg.get(0).getProductId());
            }
            pSER.updateAuctionProductImg(ProductConstant.URL_SALE_SERVER + img4Name, pImg.get(0).getId(), pImg.get(0).getProductId());
            listNames.add(img4Name);
            listFiles.add(img4);
        }
        if (img5 != null) {
            String img5Name = uploadImg.randomCodeImg() + img5.getOriginalFilename();
            pImg = pSER.getIdFromLink(getImg.get(0).getLink());
            if (pImg == null) {
                pSER.insertProductImg(ProductConstant.URL_SALE_SERVER + img5Name, pImg.get(0).getProductId());
            }
            pSER.updateAuctionProductImg(ProductConstant.URL_SALE_SERVER + img5Name, pImg.get(0).getId(), pImg.get(0).getProductId());
            listNames.add(img5Name);
            listFiles.add(img5);
        }
        try {
            uploadImg.uploadImg(listNames, listFiles, ProductConstant.URL_SALE_STORE_SERVER);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        return "redirect:/" + url + "";
...
//    
//    @RequestMapping(params = "editProduct", method = RequestMethod.GET)
//    public String editProduct(ModelMap model, HttpSession session, 
//            @RequestParam(ProductConstant.ID_PARAM) int id,
//            @RequestParam(ProductConstant.URL_PARAM) String url) {
//        model.addAttribute(ProductConstant.KEY_PRODUCT_INFO_BY_ID, pSER.getProductInfoByProductId(id));
//        model.addAttribute(ProductConstant.LIST_BRAND_KEY, pSER.getListBrand());
//        model.addAttribute(ProductConstant.LIST_CATEGORY_KEY, pSER.getListCategory());
//        model.addAttribute(ProductConstant.KEY_URL, url);
